...
namespace SierraBreeze {
    ...
    private:
        ...
        SizeGrip *sizeGrip(void) const { return m_sizeGrip; }
        //@}
    ...
    bool Decoration::isRightEdge(void) const {
        return (client().data()->isMaximizedHorizontally() || client().data()->adjacentScreenEdges().testFlag(
                Qt::RightEdge)) && !m_internalSettings->drawBorderOnMaximizedWindows();
    }

    bool Decoration::isTopEdge(void) const {
        return (client().data()->isMaximizedVertically() ||
                client().data()->adjacentScreenEdges().testFlag(Qt::TopEdge)) &&
               !m_internalSettings->drawBorderOnMaximizedWindows();
    }

    bool Decoration::isBottomEdge(void) const {
        return (client().data()->isMaximizedVertically() || client().data()->adjacentScreenEdges().testFlag(
                Qt::BottomEdge)) && !m_internalSettings->drawBorderOnMaximizedWindows();
    }
    ...
    bool Decoration::matchColorForTitleBar(void) const { return m_internalSettings->matchColorForTitleBar(); }
}
